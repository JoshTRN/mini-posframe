...
** Size
+ =mini-posframe-width= (default =60=) – width in current-frame character columns; increasing the posframe font scale does not enlarge this outer width.
+ =mini-posframe-height= (default =1=) – minimum height in lines. The frame expands vertically for multiline content and uses visual-line style word wrapping once content reaches the width limit.
+ =mini-posframe-max-height= (default =10=) – vertical limit in lines. If a prompt needs more space, mini-posframe is bypassed for that minibuffer session and the regular minibuffer remains visible. Set it to =nil= for no limit.
...
+ Width: =60= characters.
+ Minimum height: =1= line (expands to fit content).
+ Maximum height: =10= lines (larger prompts use the regular minibuffer).
